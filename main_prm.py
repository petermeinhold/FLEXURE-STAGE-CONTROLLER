...
class fiber_tip_controller(object):
   ...
   def cb(self, tim):
        ...
        for i in range(9):
            ...
            for j in range(self.n_avg):
                dsum = self.adcs[i].read_u16() + dsum
            self.adc_vals[i] = dsum//self.n_avg

        #check  preamp safe
        
        if self.adc_vals[0] < self.min_preamp_seed_d:
                self.preamp_safe=False
                self.led1.toggle()
        if self.adc_vals[1] >self.max_preamp_rev_d:
                self.preamp_safe=False
                self.led1.toggle()
...
my_controller = Amp_Controller(100)
def read_settings():
...
while True:
    ...
    if data_read:
        ...
        if data_read[0:7] == 'setstep':
            value = data_read[7:]
            value = int( value )
            dac2_step= value 
            recognized = 1
            conn.write( ("DAC 2 step set to " + str(value) + "\r\n").encode('utf-8') ) 
            
            
        if data_read[0:7] == 'setrate':
            value = data_read[7:]
            value = int( value )
            dac2_step= value 
            recognized = 1
