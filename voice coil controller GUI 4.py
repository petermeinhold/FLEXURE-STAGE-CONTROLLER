...
class Fiber_actuator( QObject ): #subclass of QObject so it has its own port, signals, and slots
    def __init__(self, port_name = 0):
        ...
        self.received_text = ''
        print('Fiber_actuator in it with  port_name = ', port_name )
        if port_name: 
...
#serial port management    
    ...
    def connection_toggle(self, checked):
        ...
        if checked:
            ...
            this_actuator = Fiber_actuator( self.port_name )
            #the application has a serial port, which is a reference to the serial port of the currently selected actuator's serial port
            #this is so all trafic can go through it without changing the use of self.port elsewhere from when there was only one element and so port was a member of the GUI not of the actuator object 9which did not exist)            
            ...
            if this_actuator.serial_number is not -1:
                ...
                print('actuator added to list, it has serial number ', this_actuator.serial_number )
            else:
    ...
    def connect_all_toggle(self, checked):
        ...
        if checked:
            ...
            for port_name in self.port_names:
                fiber_actuator = Fiber_actuator()
                fiber_actuator.connect( port_name )# = QtSerialPort.QSerialPort(port, baudRate=QtSerialPort.QSerialPort.Baud115200, readyRead=self.receive)
...
#called when main window is closed          
    def closeEvent(self, event):
        exit()
        '''
        print('closeEvent close all ports')
        for actuator in self.fiber_actuators_list:
            print('Closing port ' )
        try:
            self.serial.close()
        except:
