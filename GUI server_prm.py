...
class Ui(QtWidgets.QMainWindow):
    ...
    def set_DAC_2_step(self):
        value = int( self.lineEdit_DAC2_step.text() )
        test_str = 'setstep'  + str(value) + '\r\n'
        self.textBrowser.append("OUT: " + test_str)
        self.ser.write( test_str.encode('ascii') ) 
        line =  self.ser.readline().decode('utf-8') 
        while not line: #wait for pyboard to respond
    ...
    def set_DAC_2_rate(self):
        value = int( self.lineEdit_DAC2_rate.text() )
        test_str = 'setrate'  + str(value) + '\r\n'
        self.textBrowser.append("OUT: " + test_str)
        self.ser.write( test_str.encode('ascii') ) 
        line =  self.ser.readline().decode('utf-8') 
        while not line: #wait for pyboard to respond
